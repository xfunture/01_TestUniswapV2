...
import { APE_TOKEN, DAI_TOKEN, ERC20_ABI, MAX_FEE_PER_GAS, MAX_PRIORITY_FEE_PER_GAS, QUOTER_CONTRACT_ADDRESS, UNISWAPV3_ROUTER_ADDRESS, TOKEN_AMOUNT_TO_APPROVE_FOR_TRANSFER, UNI_TOKEN, USDC_TOKEN, WETH_TOKEN, UNISWAPV3_ROUTER2_ADDRESS } from './lib/constant';
...
async function firstDemo(){
    ...
    console.log("quotedAmountOut:",ethers.formatUnits(amountOut.toString(),CurrentConfig.tokens.out.decimals));
    /*****************************************************************************get Amount out********************************************************************* */
    ...
    // console.log(`in tokenApprovalFee: ${ethers.formatEther(approvalFee.toString())}`);
    ...
    // const amountIn = ethers.parseUnits(CurrentConfig.tokens.amountIn.toString(),18);
    // const amountInHex = amountIn.toString();
    ...
    // console.log("\n\nbefore swap")
    // console.log("eth balance:",ethers.formatEther(beforeBalance.toString()))
    ...
    // console.log("\n\nafter swap");
    // console.log("eth balance",ethers.formatEther(afterbalance.toString()));
    ...
    // console.log("daiBalance:",toReadableAmount(daiBalance,DAI_TOKEN.decimals));
    // console.log("uniBalance:",toReadableAmount(uniBalance,UNI_TOKEN.decimals));
    // console.log(`transferFeeEther: ${ethers.formatEther(transferFeeEther.toString())}`);
...
async function testMultiHopSwap(){
    ...
    console.log("amountIn:",ethers.parseUnits(amountIn.toString(),tokenIn.decimals));
    ...
    const rawTxn = await UNISWAPV3_ROUTER_CONTRACT.exactInputSingle.populateTransaction(
        params,
        {
            gasLimit:1000000,
...
async function testMultiHopSwapV2(){
    ...
    console.log(`tokenIn:${tokenIn.symbol} tokenOut:${tokenOut.symbol} approve fee:${ethers.formatUnits(approvalFee,tokenIn.decimals)}`);
